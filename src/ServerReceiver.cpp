...
void ServerReceiver::threadFun() {
  ...
  for(;;){
    ...
    if(Packet::checkPacketIntegrity(input_raw_packet, max_seq_digits, false)){
      ...
      //std::cout << "raw_packet: '" << input_raw_packet << "'" << std::endl;
      //std::cout << "CHECKSUM OK: " << packet_seq_num << ". max_seq_digits: " << max_seq_digits << std::endl;
      ...
      //std::cout << "ServerReceived, extractedData: " << extractedData << std::endl;
    ...
    //std::cout <<  input_raw_packet << std::endl;
  }
...
ServerReceiver::~ServerReceiver() {
  join();
  std::cout << "ServerReceiver finished from joining" << std::endl;
}
